...
if "anamnese_pronta" not in st.session_state:
    ...
    with st.sidebar:
        ...
        # 🗺️ MENU INTERATIVO DE ABAS (O clique muda a tela!)
        aba_selecionada = st.radio(
            "🗺️ Navegação",
            ["🆕 Nova Consulta", "📂 Pacientes"],
            index=0
        )
...
if   aba_selecionada == "🆕 Nova Consulta":
    st.title("🩺 AnamneMed")
                ...
                with st.spinner("A Inteligência Artificial está organizando os dados médicos..."):
                    try:
                        ...
                        *REGRA CRUCIAL DE OMISSÃO:* Se o médico não tiver mencionado absolutamente nenhuma informação sobre antecedentes (sejam familiares, pessoais ou patologias passadas), NÃO exiba esta seção de forma alguma. O título e os tópicos devem sumir completamente do resultado final.
                        ...
                        if st.button("📁 Salvar no Histórico da Sessão", use_container_width=True):
                            import datetime
                        agora = datetime.datetime.now()
                        dia_atual = agora.strftime("%d/%m/%Y") # Ex: 09/06/2026
                        hora_atual = agora.strftime("%H:%M")   # Ex: 14:30
                        
                        # 🧠 Mini-IA Algorítmica: Lê o texto pronto e extrai os dados limpos
                        t_limpo = resultado_real.replace("**", "").replace("-", "")
                        nome, idade, sexo, qp = "Paciente Sem Nome", "", "", ""
                        
                        for linha in t_limpo.split("\n"):
                            if "nome" in linha.lower() and ":" in linha: 
                                nome = linha.split(":", 1)[1].strip()
                            elif "idade" in linha.lower() and ":" in linha: 
                                idade = linha.split(":", 1)[1].strip()
                            elif "sexo" in linha.lower() and ":" in linha: 
                                sexo = linha.split(":", 1)[1].strip()
                            elif ("queixa principal" in linha.lower() or "qp:" in linha.lower()) and ":" in linha: 
                                qp = linha.split(":", 1)[1].strip()
                        
                        # Monta o resumo idêntico ao estilo do Gemini
                        resumo_estilo_gemini = f"👤 {nome}"
                        if status_idade := idade: resumo_estilo_gemini += f", {status_idade}"
                        if status_sexo := sexo:   resumo_estilo_gemini += f" ({status_sexo})"
                        if status_qp := qp:       resumo_estilo_gemini += f" ➜ {status_qp}"
                        
                        # Guarda tudo estruturado na memória
                        st.session_state.historico_pacientes.append({
                                "dia": dia_atual,
                                "hora": hora_atual,
                                "resumo": resumo_estilo_gemini,
                                "conteudo": resultado_real
                            })
                        st.success("Arquivado automaticamente com sucesso!")
